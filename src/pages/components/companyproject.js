...
const CompanyProject  = ({title,dataId,tools,description,setId}) =>{
    ...
    return(
       ...
       <div   className="h-[18rem] glass w-[25rem]   shadow-xl  hover:transition hover:ease-in-out hover:duration-[1000] hover:delay-100 hover:scale-125
                ...
                <h1 className="text-black text-lg  sm:max-lg:p-4   text-center font-serif ">{title}</h1>
                    <h1 id={dataId} className="h-24 w-full text-black text-md  text-clip  sm:max-lg:text-sm font-serif  text-center">{description}</h1>
                </div>
               ...
               flex flex-row flex-wrap justify-center items-center gap-2">
                    ...
                    tools &&  tools.map((e,i)=>{
                            return(
                                <button key={i} className="p-2 text-black text-md rounded-md border border-gray-300 font-serif  ">{e}</button>  
                            )
                        })
                    }
                </div>    
